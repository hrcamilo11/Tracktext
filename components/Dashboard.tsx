...
export default function TextileDashboard() {
  ...
  const handleNewClientSubmit = async () => {
    ...
    const response = await fetch('http://localhost:5000/api/auth/register', {
      ...
      body: JSON.stringify({ username: newClient.username, password: newClient.password, email: newClient.email, phone: newClient.phone, role: 'client' }),
    });
  ...
  const handleNewEmployeeSubmit = async () => {
    ...
    const response = await fetch('http://localhost:5000/api/auth/register', {
      ...
      body: JSON.stringify({ username: newEmployee.username, password: newEmployee.password, email: newEmployee.email, phone: newEmployee.phone, role: 'employee' }),
    });
  ...
  const handleEmployeeDelete = async (employeeId: string) => {
    ...
    toast.info("Se ha eliminado el elmpleado exitosamente.")
    }
