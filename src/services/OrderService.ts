...
export class OrderService {
    ...
    async createOrder(client: string, product: string, size: string, reference: number, quantity: number, dueDate: string, status: string, progress: {
        [key: string]: { completed: number }
    }): Promise<IOrder> {
        ...
        const order = await new Order({client, product, size, reference, quantity, dueDate, status, progress});
